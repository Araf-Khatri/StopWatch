...
function timerStart() {
    if (!this.classList.contains("resume")) changeClass();
    else stopTimer();
    start();
}
const resetHTML = function() {
    ...
    buttons.forEach((btn)=>{
        if (!btn.classList.contains("start")) btn.classList.add("hidden");
...
const stopTimer = function() {
    ...
    buttons.forEach((btn)=>{
        if (!btn.classList.contains("start")) btn.classList.toggle("hidden");
    });
};
const checkGap = function(arr) {
    ...
    if (arr.length !== 1) prev = arr[arr.length - 2].split(":");
...
},{}]},["2mNKm","6rimH"], "6rimH", "parcelRequirea69d")
